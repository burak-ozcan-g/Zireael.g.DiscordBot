...
module.exports.run = async (bot, message, args) => {
    ...
    if(!helpArgs[0]) {
        var embed = new Discord.MessageEmbed()
            .setAuthor(`Kullanılabilir komutlar`)
            .setDescription('```Aşağıdaki, yetkisine sahip olduğunuz komutları kullanabilirsiniz.```')
            .addFields(
                { name: 'Prefix', value: '```z.```', inline: true},
                { name: 'Moderasyon', value: '```| at | kaldır | temizle | yasakla | yatır |```'},
                { name: 'Bilgi', value: '```| üyebilgi | yardım |```'})
            .setColor('#00FFF3')
